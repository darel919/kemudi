...
The MFD publishes FCW state, target detection, target distance, signed relative speed, time-to-collision, sensor confidence, sensor faults, AEB state, and the normalized AEB brake command from the authoritative worker telemetry. Following front-crash-prevention behavior, FCW warns before automatic braking; once AEB activates it suppresses propulsion, applies a strong initial brake command that increases toward the available tire/ABS limit as impact becomes imminent, and loops the FCW beep from `/assets/sounds/beep_generic_fcw.ogg` until the intervention ends. AEB remains available at low speed but releases FCW/AEB state once the vehicle is effectively stationary after an impact, preventing stale closing-speed data from holding brake pressure. It cannot override tire friction or claim that every collision is avoidable.
