...
- **Beam:** endpoint node IDs, rest length, axial stiffness in N/m, tensile yield/failure loads in N, damping, broken state, XPBD lambda, and measured axial stress. Vehicle files may optionally author `yieldStrength` and `plasticity`; omitted values default to 55% of failure strength and 0.25 respectively.
...
Suspension `antiRollBarStiffness` is interpreted as N/m and is applied to physical left/right wheel deflection, not normalized compression. Damper velocity is derived from the change in terrain-to-mount suspension length, so moving over a changing terrain height contributes to compression/rebound; the derived shaft rate is bounded so discrete height-field and rut samples cannot inject an unphysical launch impulse. Suspension and tire forces are projected onto the sampled terrain normal, while rolling and lateral velocities use the corresponding tangent basis. Positive rut depth lowers the height field. The render assembly keeps each wheel at its authoritative suspension contact, steers the front wheel pivots from telemetry, and spins each tire from its measured rolling speed.
...
Map loading passes the terrain layer's default built-in material and the map-authored base friction into the worker. An automatic transmission also observes the driver's requested throttle rather than a traction-control-reduced actuator command, holds first gear through the low-speed launch phase, and downshifts out of an accidentally tall launch gear instead of treating converter flare as an upshift request.
...
Driven-wheel torque crosses into the chassis exactly once through the grounded tire force. While the requested torque fits inside the available traction, static contact constrains the driven shaft to the same-step ground-relative rolling speed; this keeps automatic-transmission crawl from under-rotating the driven tires as the chassis accelerates. Once the contact patch saturates, only the transmitted reaction torque is returned to the shaft and the untransmitted torque remains as physical wheelspin. With zero throttle, static contact likewise synchronizes the driven wheel instead of retaining an independently integrated vehicle velocity. Engine and service braking are dissipative: their torque is bounded by the angular momentum available in the fixed step, and contact force is bounded so it can stop but not reverse the chassis. Service-brake and VSC forces are additionally limited by each wheel's remaining friction-circle capacity after lateral and drive forces. Service-brake torque is not included in `last_drive_torque`, because braking is applied separately at the grounded wheel contacts. Fuel mass is distributed across dynamic nodes as a deterministic approximation, and authored unsprung masses configure driven-wheel rotational inertia.

Forward-collision ADAS uses the same map collision primitives and boundaries that are sent to the physics worker, in addition to remote vehicle snapshots. Boxes are projected into the vehicle's forward sensor corridor and spheres use their radius as the detection envelope. A static obstacle reports a negative relative speed equal to the vehicle's closing speed; remote targets report `targetSpeed - egoSpeed`, matching the Rust controller's closing-speed convention. This lets FCW/AEB respond to walls, boulders, and other static obstacles instead of only moving multiplayer vehicles. Detection confidence remains available at low and moderate speeds; time-to-collision and brake authority still bound intervention, so ADAS does not create grip or guarantee that a collision can be avoided.

The MFD publishes FCW state, target detection, target distance, signed relative speed, time-to-collision, sensor confidence, sensor faults, AEB state, and the normalized AEB brake command from the authoritative worker telemetry. Following front-crash-prevention behavior, FCW warns before automatic braking; a closing target must persist for a short confirmation window before FCW/AEB can intervene, with the FCW lead window expanding from 2.5 to 3.1 seconds at higher speed. AEB uses a speed-adaptive TTC engagement band from 0.8 to 2.0 seconds, starts with partial braking, and selects the larger of a smooth TTC demand or the deceleration required to remove the measured closing speed over the available gap. This gives low-speed events a less abrupt initial response while allowing high-speed threats to begin earlier. The FCW beep from `/assets/sounds/beep_generic_fcw.ogg` loops only while intervention is active. AEB remains available at low speed but releases FCW/AEB state once the vehicle is effectively stationary after an impact, preventing stale closing-speed data from holding brake pressure. It cannot override tire friction or claim that every collision is avoidable.
...
Beam compliance and accumulated constraint lambdas provide XPBD-style stability. Beam dampers are applied once as an implicit axial velocity impulse after positional iterations, so damping remains dissipative and does not scale with the number of solver iterations. Tensile load follows an elastic/yield/break path: once the XPBD axial load exceeds the beam's yield threshold, a bounded fraction of excess strain permanently increases its rest length; loads above strength break it. Compression is tracked but does not shorten a scalar beam, avoiding unstable buckling feedback without a dedicated bending model. Triangle area constraints use the derived area gradients and inverse-mass-weighted XPBD corrections rather than centroid scaling, so fixed and differently weighted vertices respond consistently. Parameters are clamped, invalid states are recovered, and deterministic reset behavior is covered by Rust tests. Solver iteration tuning is still a performance follow-up; graphics presets do not change authoritative controls or collision rules.
